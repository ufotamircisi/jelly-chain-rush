...
export const REGEN_ENERGY_CAP = 300;
export const REGEN_START_THRESHOLD = 99;
...
export function applyFreeEnergy(currentEnergy: number, rewardEnergy: number): { energy: number; gained: number; capped: boolean } {
  const nextEnergy = currentEnergy + rewardEnergy;
  return { energy: nextEnergy, gained: rewardEnergy, capped: false };
}

export function applyMarketEnergy(currentEnergy: number, purchasedEnergy: number): { energy: number; gained: number; capped: boolean } {
  const nextEnergy = currentEnergy + purchasedEnergy;
  return { energy: nextEnergy, gained: purchasedEnergy, capped: false };
}
