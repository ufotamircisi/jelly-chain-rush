...
$assets = @(
  @{ Name = 'logo'; Path = 'src/assets/ui/logo/jelly_chain_rush_logo.png'; Width = 900; Height = 300; MinFill = 70; MaxCenterOffset = 8 },
  @{ Name = 'nav_play'; Path = 'src/assets/ui/nav/nav_icon_play.png'; Width = 256; Height = 256; MinFill = 55; MaxCenterOffset = 10 },
  @{ Name = 'nav_island'; Path = 'src/assets/ui/nav/nav_icon_island.png'; Width = 256; Height = 256; MinFill = 70; MaxCenterOffset = 10 },
  @{ Name = 'nav_market'; Path = 'src/assets/ui/nav/nav_icon_market.png'; Width = 256; Height = 256; MinFill = 70; MaxCenterOffset = 10 },
  @{ Name = 'stat_energy'; Path = 'src/assets/ui/stats/icon_energy.png'; Width = 256; Height = 256; MinFill = 60; MaxCenterOffset = 10 },
  @{ Name = 'stat_diamond'; Path = 'src/assets/ui/stats/icon_diamond.png'; Width = 256; Height = 256; MinFill = 70; MaxCenterOffset = 10 },
  @{ Name = 'stat_shake'; Path = 'src/assets/ui/stats/icon_shake.png'; Width = 256; Height = 256; MinFill = 70; MaxCenterOffset = 10 }
)
...
foreach ($asset in $assets) {
  $report = $reports | Where-Object { $_.Name -eq $asset.Name } | Select-Object -First 1
  if ($null -eq $report -or $report.Dimensions -ne "$($asset.Width)x$($asset.Height)") {
    if ($null -ne $report) {
      $failures += $report
    }
  }
...
if ($failures.Count -gt 0) {
  throw "UI asset validation failed: $($failures.Count) invalid file(s)."
}

$warnings = @($reports | Where-Object { $_.Warnings })
if ($warnings.Count -gt 0) {
  Write-Warning "UI asset validation completed with $($warnings.Count) warning(s)."
} else {
  Write-Output 'UI asset validation completed with no warnings.'
}
